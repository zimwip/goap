...
func newResolver(nodes []domain.Node, change domain.ChangeSet, newID func() string) *resolver {
	r := &resolver{byKey: map[string]domain.NodeRef{}, byID: map[domain.NodeID]domain.NodeRef{}, items: map[domain.ItemID]bool{}, local: map[string]domain.ItemID{}, newID: newID}
	for _, n := range nodes {
		r.byKey[n.Key] = n.Ref()
		r.byID[n.ID] = n.Ref()
	}
